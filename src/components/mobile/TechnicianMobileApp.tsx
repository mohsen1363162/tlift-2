...
export default function TechnicianMobileApp({
  ...
  const submitPayment = (skip: boolean) => {
    ...
    setPayModal(false);
    notify(skip ? "سرویس ثبت شد" : "سرویس و دریافت وجه ثبت شد");
    resetWork();
    setSelected(null);
    setScreen("home");
  };
  ...
  const header = (title?: string, back?: () => void) => (
    <div className="flex items-center justify-between bg-white px-3 py-2 shadow-sm">
      ...
      <div className="text-[14px] font-bold text-gray-800">{title || "تلیفت همراه"}</div>
      <button
        type="button"
        onClick={toggleDay}
        className={`flex items-center gap-1 rounded-full px-3 py-1.5 text-[12px] font-bold text-white ${
          dayStart ? "bg-blue-600" : "bg-emerald-600"
        }`}
      >
        {dayStart ? (
          <>
  ...
  const renderSignView = () => (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 mx-auto max-w-[480px] p-3">
        <button
          type="button"
